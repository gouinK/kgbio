...
def clonality_boxplot(adata=None, df=None, groupby=None, rep=None, xcat=None, hcat=None,
                        ...
                        fontsize=4, ylim=None, figsize=None):
    
    if calc_pct:
        df = clonality_df(adata, groupby=groupby, rep=rep, xcat=xcat, metrics=metrics, drop_na=drop_na, receptor_column=receptor_column)
...
def clonality_lineplot(adata=None, df=None, rep=None, xcat=None, hcat=None,
    ...
    if calc_pct:
        df = clonality_df(adata, groupby=None, rep=rep, xcat=xcat, metrics=metrics, drop_na=drop_na, receptor_column=receptor_column)
...
## TCR/BCR sharing
def receptor_sharing(adata=None, df=None, calc=True, groupby=None, fontsize=2, figsize=None, vmax=None):
    ...
    if calc:
        ...
        df = pd.DataFrame(columns=['percent','from','to','treatment'])

        for t in tx:
            
            for p1 in pt:
                
                total = adata[(adata.obs.cohort==p1)&(adata.obs.treatment==t)]
                r = total.obs[groupby].unique().tolist()
                num_r = len(r)
                ...
                for p2 in pt:
                    ...
                    total2 = adata[(adata.obs.cohort==p2)&(adata.obs.treatment==t)]
                    r2 = total2.obs[groupby].unique().tolist()
    ...
    tx = ['Base','PD1','RTPD1']
    pt = adata[(adata.obs.pCR=='R')].obs.cohort.unique().tolist() + adata[(adata.obs.pCR=='NR')].obs.cohort.unique().tolist()
    ...
    for t,ax in zip(tx,axs.flat):
        ...
        _= sns.heatmap(tmp,ax=ax,vmax=vmax)
        _= ax.set(title=t)
...
def vdj_usage_heatmap(adata=None,df=None,calc_pct=True,comparator=None,reference=None,logic_comp='and',logic_ref='and',
    ...
    if calc_pct:
        df = pf.pct_df(tmp,groupby=groupby,rep=rep,xcat=xcat,drop_na=drop_na,thresh=thresh,normalization=normalization,receptor_column=receptor_column)
